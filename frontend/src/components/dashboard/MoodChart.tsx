export function MoodChart({ data }: { data?: Array<{ day: string; mood: string; score: number }> }) {
 ...
 <div className="h-3.5 overflow-hidden rounded-full bg-surface border border-sky/40 p-[2px] shadow-inner">
 <div 
 ...
 <span className="text-right text-muted font-semibold tracking-wide bg-slate-100/60 rounded px-2 py-0.5 border border-slate-200/50">
 {item.mood}
 </span>
 </div>
 ))}
