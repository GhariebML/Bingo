  ...
  return (
    <section className="grid gap-6 lg:grid-cols-[0.72fr_0.28fr]">
      <div className="space-y-5">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-calm">Bingo chat</p>
          <h1 className="mt-2 text-4xl font-bold text-ocean">A gentle place to start</h1>
      ...
      <aside className="space-y-4">
        <Card className="overflow-hidden p-0">
          ...
          <div className="p-5">
            <h2 className="font-semibold text-ocean">Bingo is listening</h2>
            <p className="mt-2 text-sm leading-6 text-slate-700">The chat now sends messages to the local FastAPI backend and uses the safe mock provider.</p>
          </div>
        </Card>
        <Card title="Quick starts">
          <div className="grid gap-2">
            {quickStarts.map((prompt) => (
              <div key={prompt} className="rounded-lg border border-sky bg-foam px-3 py-3 text-left text-sm font-semibold text-ocean">
                {prompt}
              </div>
            ))}
          </div>
