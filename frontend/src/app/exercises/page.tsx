      ...
      <Card className="overflow-hidden p-0 border border-white/50 shadow-soft">
        <div className="grid items-center gap-6 md:grid-cols-[0.4fr_0.6fr]">
          ...
          <div className="p-6 space-y-2 select-none">
            <div className="flex items-center gap-2 text-calm font-bold text-sm uppercase tracking-wider">
          ...
          return (
            <Card 
              ...
              className={`flex flex-col justify-between transition-all duration-500 ${
                isActive ? 'ring-2 ring-calm/10 border-calm/20 scale-[1.01]' : ''
              }`}
            >
              <div className="space-y-4">
                {/* Meta Tags */}
                <div className="flex flex-wrap gap-2 text-[10px] font-extrabold uppercase tracking-wider select-none">
                  <span className="flex items-center gap-1 rounded-full bg-sky/60 px-3 py-1 text-ocean border border-white/60">
                    <Clock size={11} />
                    {exercise.duration_minutes} Min
                  </span>
                  <span className="flex items-center gap-1 rounded-full bg-mint/50 px-3 py-1 text-ocean border border-white/60">
                    <Target size={11} />
                    {exercise.purpose}
                  </span>
                </div>
                ...
                <ol className="space-y-3.5">
                  {(isActive ? exercise.steps : exercise.steps.slice(0, 2)).map((step, index) => (
                    <li key={step} className="flex gap-3 text-sm leading-relaxed text-slate-600 transition-opacity duration-300">
                      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-sky to-mint/40 text-xs font-bold text-ocean border border-white shadow-sm select-none">
                        {index + 1}
                      </span>
              ...
              {/* Action Button */}
              <div className="mt-6 pt-4 border-t border-ocean/5 flex justify-end">
                <Button 
